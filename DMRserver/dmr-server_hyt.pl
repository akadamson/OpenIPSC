...
while($Frame = <STDIN>) {
	...
	if ($Status == 1) {
		$Query = "INSERT INTO `dmrdb`.`UserLog` (`StartTime`, `EndTime`, `SourceNet`, `TimeSlot`, `RepeaterID`, `DmrID`, `DestinationID` ) VALUES('$DateTime','0000-00-00 00:00:00','$SourceNet', '$Slot','$RepeaterIP', '$SourceID', '$DestinationID');\n";
	print "$Query";
	#} elsif ($Status == 0) {
